...
public class MapDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(/*Graphics gc*/ GraphicsWrapper panel, int state, int px, int py,
			int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang, boolean stopped, float battery, int path) {
		//dbg("redraw") ;
		if (state != Constants.STATE_PLAY)
			return ;
		if (maze.isInMapMode()) {
			draw_map(/*gc*/panel, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
			draw_currentlocation(/*gc*/panel, view_dx, view_dy) ;
		}
	}
	/**
	 ...
	 */
	public void draw_map(/*Graphics gc*/GraphicsWrapper panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				/*gc*/panel./*setColor*/WrapperSetColor(seencells.hasWallOnTop(x, y) ? /*Color.white*/WrapperColor.white : /*Color.gray*/WrapperColor.gray);
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					/*gc*/panel./*drawLine*/WrapperDrawLine(nx1, ny1, nx2, ny1);
				...
				/*gc*/panel./*setColor*/WrapperSetColor(seencells.hasWallOnLeft(x, y) ? /*Color.white*/WrapperColor.white : /*Color.gray*/WrapperColor.gray);
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					/*gc*/panel./*drawLine*/WrapperDrawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
			//draw_solution(gc, offx, offy);
			draw_solution(/*gc*/panel, offx, offy, px, py) ;
		}
	 ...
	 * @param gc
	 */
	public void draw_currentlocation(/*Graphics gc*/GraphicsWrapper panel, int view_dx, int view_dy) {
		...
		/*gc*/panel./*fillOval*/WrapperFillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		/*gc*/panel./*drawLine*/WrapperDrawLine(ctrx, ctry, aptx, apty);
		/*gc*/panel./*drawLine*/WrapperDrawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		/*gc*/panel./*drawLine*/WrapperDrawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	 ...
	 * @param offy
	 */
	public void draw_solution(/*Graphics gc*/ GraphicsWrapper panel, int offx, int offy, int px, int py) {
		// check parameters:
		...
		while (d > 1) {
			...
			// do the graphics
			/*gc*/panel./*drawLine*/WrapperDrawLine(nx1, ny1, nx1+ndx, ny1+ndy);
