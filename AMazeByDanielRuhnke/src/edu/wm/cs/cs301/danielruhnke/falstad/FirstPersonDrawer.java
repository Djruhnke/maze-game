...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	@Override
	public void redraw(/*Graphics gc*/ GraphicsWrapper panel, int state, int px, int py, int view_dx, int view_dy, int walk_step, 
			int view_offset, RangeSet rset, int ang, boolean stopped, float battery, int path) {
		...
		/*gc*/panel./*fillRect*/WrapperFillRect(0, 0, view_width, view_height/2);
		// draw dark gray background on upper half
		/*gc*/panel./*setColor*/WrapperSetColor(/*Color.darkGray*/WrapperColor.darkGray);
		/*gc*/panel./*fillRect*/WrapperFillRect(0, view_height/2, view_width, view_height/2);
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		/*gc*/panel./*setColor*/WrapperSetColor(seg.col.getRGB());
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			/*gc*/panel./*fillPolygon*/WrapperFillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
