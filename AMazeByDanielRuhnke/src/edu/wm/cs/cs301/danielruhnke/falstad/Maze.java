...
import edu.wm.cs.cs301.danielruhnke.ui.GeneratingActivity;
import edu.wm.cs.cs301.danielruhnke.ui.PlayActivity;
import edu.wm.cs.cs301.danielruhnke.ui.R;
...
public class Maze  {
	...
	//Make maze aware of robot
	public int pathLength = 0;
	public float battery = 0;
	public boolean stopped = false;
	
	//Make maze aware of JPanel
	...
	GeneratingActivity generateActivity;
	...
	public Maze(GeneratingActivity activity){
		super();
		generateActivity = activity;
	}
	public Maze(GeneratingActivity activity, MazeFileReader mazefile){
		super();
		generateActivity = activity;
		mazew = mazefile.width;
		mazeh = mazefile.height;
		mazecells = mazefile.cells;
		rootnode = mazefile.root;
		mazedists = new Distance(mazefile.dists);
		px = mazefile.startx;
		py = mazefile.starty;
		dx = 1;
		dy = 0;
		newMaze(rootnode, mazecells, mazedists, px, py);
	}
	/**
	 ...
	 */
	public void build(int skill, String mazeBuild) {
		// switch screen
		...
		//Set generation method based on dropdown boxes
		if (mazeBuild.equals("Backtracking")){
			mazebuilder = new MazeBuilder();
		}
		
		else if (mazeBuild.equals("Eller\'s")){
			mazebuilder = new MazeBuilderEller();
		}
		
		else if (mazeBuild.equals("Prim\'s")){
			mazebuilder = new MazeBuilderPrim();
		}
		else if (mazeBuild.equals("Preloaded Maze")){
			//TODO implement this
			mazebuilder = new MazeBuilder();
		}
	...
	public void newMaze(BSPNode root, Cells c, Distance dists, int startx, int starty) {
		...
		generateActivity.revealButtons();
		// notify viewers
		//notifyViewerRedraw() ;
	}
	...
	public void notifyViewerRedraw() {
		...
		while (it.hasNext())
		{
			...
			//panel.setWrapperGraphics();
			v.redraw(panel/*.getBufferGraphics()*/, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, 
					rset, angle, stopped, 2500 - battery, pathLength) ;
		}
		...
		if (state == Constants.STATE_FINISH){
			if (stopped){
				playActivity.onShortcutFailure();
			}
			else{
				playActivity.onShortcutSuccess();
			}
		}
	}
	/** 
	...
	public boolean increasePercentage(int pc) {
		if (percentdone < pc && pc < 100) {
			...
			{
				//notifyViewerRedraw() ;
				generateActivity.incrementProgress(percentdone);
			}
			else
	 ...
	 */
	public boolean keyDown(DirectionEvents key) {
		...
		case Constants.STATE_PLAY:
			...
			case /*Event.UP*/UP:
				walk(1);
				break;
			case /*Event.LEFT*/LEFT:
				rotate(1);
				break;
			case /*Event.RIGHT*/RIGHT:
				rotate(-1);
				break;
			case /*Event.DOWN*/DOWN:
				walk(-1);
				break;
