...
import edu.wm.cs.cs301.danielruhnke.falstad.MazeFileReader;
...
public class GeneratingActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!builder.equals("Preloaded Maze")){
			maze = new Maze(this);
			//maze.init();
			maze.build(skill, builder);
		}
		else{
			String filename = getFilesDir().toString() + "/skill_" + Integer.toString(skill);
			File check = new File(filename);
			if (!check.exists()){
				Log.v("No Save", "Building new maze");
				maze = new Maze(this);
				maze.build(skill, builder);
			}
			else{
				MazeFileReader mazefile = new MazeFileReader(filename);
				maze = new Maze(this, mazefile);
